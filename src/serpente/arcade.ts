...
const CICLO_CAPA = 6000;
const TAU = Math.PI * 2;

function ease(v: number): number { const n = Math.max(0, Math.min(1, v)); return n * n * (3 - 2 * n); }
function entre(p: number, inicio: number, fim: number): number { return ease((p - inicio) / (fim - inicio)); }
function janela(p: number, entrada: number, cheio: number, saida: number, fim: number): number {
  return Math.min(entre(p, entrada, cheio), 1 - entre(p, saida, fim));
}
...
/** A cobra percorre um oito fechado: posição, velocidade e luz coincidem no fecho. */
const cenaSerpente: Cena = (ctx, l, a, t, h) => {
  ...
  const onda = (s: number): { x: number; y: number } => {
    const f = ((s % 1) + 1) % 1;
    return { x: l * (.5 + Math.sin(f * TAU) * .39), y: a * (.5 + Math.sin(f * TAU * 2) * .22) };
  };
  const cabecaS = t / CICLO_CAPA;
  const corpo = 11;
  ...
  const luz = { x: l * .5, y: a * .5 }, pulso = .82 + Math.sin(cabecaS * TAU * 3) * .18;
...
const cenaMaze: Cena = (ctx, l, a, t, h) => {
  ...
  const vaivem = (fase: number): number => .5 - Math.cos((t / CICLO_CAPA + fase) * TAU) * .5;
...
const cenaMinas: Cena = (ctx, l, a, t, h) => {
  ...
  const foco = { x: 2.5, y: 2 }, alcance = 7.2;
  const fase = (.5 - Math.cos((t / CICLO_CAPA) * TAU) * .5) * 1.55;
...
const cena2048: Cena = (ctx, l, a, t, h) => {
  fundo(ctx, l, a, h);
  const cel = Math.min(l * .26, a * .4), ciclo = t / CICLO_CAPA;
  ...
  const retorno = entre(ciclo, .84, .94);
  const avanco = entre(ciclo, .08, .48) * (1 - retorno);
  const alphaDois = Math.max(1 - entre(ciclo, .46, .54), entre(ciclo, .94, .995));
  const alphaResultado = janela(ciclo, .46, .54, .78, .88);
  ctx.globalAlpha = alphaDois;
  const afastamento = (1 - avanco) * l * .2;
  peca(centro - cel * .54 - afastamento, 64, 1, h);
  peca(centro + cel * .54 + afastamento, 64, 1, h);
  ctx.globalAlpha = alphaResultado;
  if (alphaResultado > 0) {
    const impacto = entre(ciclo, .48, .7);
...
const cenaTetris: Cena = (ctx, l, a, t, h) => {
  ...
  const flash = janela(ciclo, .5, .56, .66, .76);
  for (let x = 0; x < colunas; x++) for (let i = 0; i < alturas[x]; i++) {
    const y = linhas - 1 - i;
    bloco(x, y, (h + x * 9) % 360, y === linhas - 1 ? flash : 0);
  }
  // A peça em queda fecha o buraco da coluna do meio.
  const queda = entre(ciclo, .06, .52);
  const y = (linhas - 3) * queda;
  ctx.globalAlpha = janela(ciclo, .02, .06, .62, .76);
  bloco(4, y + 1, (h + 46) % 360, flash);
  bloco(4, y, (h + 46) % 360, flash);
  ctx.globalAlpha = 1;
};
...
const cenaPrisma: Cena = (ctx, l, a, t, h) => {
  ...
  for (let fila = 0; fila < filas; fila++) for (let coluna = 0; coluna < colunas; coluna++) {
    ...
    const ciclo = .5 - Math.cos((t / CICLO_CAPA) * TAU) * .5;
...
function quadroCapas(agora: number): void {
  ...
  for (const capa of capas) {
    ...
    medir(capa);
    const tempo = parado ? CICLO_CAPA * .18 : agora % CICLO_CAPA;
    CENAS[capa.id](capa.ctx, capa.tela.width, capa.tela.height, tempo, ficha(capa.id).matiz);
  }
