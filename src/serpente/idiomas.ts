...
type Textos = { [K in keyof typeof pt]: string };
export type ChaveTexto = keyof Textos;
...
export function t(chave: keyof Textos, variaveis: Record<string, string | number> = {}): string {
  return Object.entries(variaveis).reduce((texto, [nome, valor]) => texto.replaceAll(`{${nome}}`, String(valor)), DICIONARIOS[actual][chave]);
}
