import { Tetris, blocos, COLUNAS, LINHAS, OCULTAS, type Peca, type TipoPeca } from './logica';
...
export function montarTetris(aoMenu: () => void): { activar(): void; desactivar(): void } {
  ...
  function resultado(nome: string, terminou: boolean, nivelSubiu: boolean): void {
  ...
  function accao(tipo: string): void {
    ...
    else if (tipo === 'hold') ok = jogo.reservar(); else if (tipo === 'drop') { const r = jogo.quedaTotal(); resultado(r.nome, r.terminou, r.nivelSubiu); ok = true; }
  ...
  function quadro(agora: number): void {
    ...
    if (activo) {
      const r = jogo.actualizar(dt); if (r) resultado(r.nome, r.terminou, r.nivelSubiu);
