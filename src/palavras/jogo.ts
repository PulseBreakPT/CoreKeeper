...
function Faixas(): string {
  return `<div class="jg-faixa roxa"><span class="jg-faixa-icone">${Icon({nome:'alvo'})}</span><span class="jg-faixa-corpo"><small>RITMO DO RECORDE</small><b id="fantasma-recorde">PRIMEIRO PERCURSO</b><span class="jg-barra"><i id="fantasma-barra"></i></span></span></div><div class="jg-faixa azul"><span class="jg-faixa-icone">${Icon({nome:'balao'})}</span><span class="jg-faixa-corpo"><small>ÚLTIMAS RESPOSTAS</small><div class="jg-historico" id="historico" aria-label="Respostas anteriores"></div></span></div>
...
function Paineis(): string {
  return `<div class="jg-feedback" id="feedback" role="status" hidden><strong id="feedback-titulo">CERTO</strong><span id="feedback-texto" data-testid="feedback-text">+145 PONTOS</span></div><div class="jg-contagem" id="contagem" hidden><small id="contagem-modo">PLURAL</small><span>PREPARA-TE</span><b id="contagem-numero">3</b></div>
  <section class="jg-painel" id="painel" data-testid="results-dialog" role="dialog" aria-modal="true" aria-labelledby="painel-texto" hidden><div class="jg-painel-caixa"><small id="painel-etiqueta">PARTIDA TERMINADA</small><h2 id="painel-titulo">0</h2><p id="painel-texto">NOVO RECORDE</p><div class="jg-medalha" id="medalha-perfeita" hidden>${Icon({nome:'trophy'})} PERFECT RUN</div><div class="jg-resumo">${[['certas','CERTAS'],['erradas','ERRADAS'],['combo','MELHOR COMBO'],['tempo','TEMPO MÉDIO']].map(([id,label])=>`<span><b id="resumo-${id}">0</b><small>${label}</small></span>`).join('')}</div><button class="nw-cta jg-painel-cta" id="jogar-novamente" data-testid="play-again-button"><strong>JOGAR OUTRA VEZ</strong><i>${Icon({nome:'play'})}</i></button><div class="jg-painel-linha"><button class="jg-botao-claro" id="rever-erros" data-testid="review-errors-button">${Icon({nome:'balao'})}<b>REVER ERROS</b></button><button class="jg-botao-claro" id="ir-menu" data-testid="results-menu-button">${Icon({nome:'sair'})}<b>OUTRO MODO</b></button></div></div></section>
  <section class="jg-revisao" id="revisao" data-testid="review-dialog" role="dialog" aria-modal="true" aria-labelledby="review-title" hidden><div class="jg-revisao-caixa"><header><button id="fechar-revisao" data-testid="review-close" aria-label="Fechar revisão">${Icon({nome:'chevron',classe:'voltar'})}</button><span><small>DESEMPENHO</small><h2 id="review-title">Palavras a rever</h2></span></header><div id="lista-erros"></div></div></section>`;
}
